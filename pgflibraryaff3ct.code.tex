...
% #1 node options (can be "null")
% #2 id {ti}
% #3 position {x,y} (can be "null")
...
% #8 a list of attributes (can be {"rev", "seq", "null"})
\newcommand{\task}[8]{
  % parse arguments
  \def\tskid{#2}
  \def\tsklabl{#4}
  ...
  % draw task
  \def\tskpos{#3}
  \ifthenelse{\equal{\tskpos}{\string "null"}} {
    \ifthenelse{\equal{#1}{\string "null"}} {
