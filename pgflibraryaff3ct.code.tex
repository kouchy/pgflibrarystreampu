...
\usepackage{tikz}
\usetikzlibrary{matrix, positioning, fit, patterns, shapes, arrows, shapes.multipart, decorations.pathmorphing, calc, shapes.geometric}
...
\tikzset{ task/.style   = {draw=Paired-1,  rounded corners=0pt, text=Paired-1, minimum height=\tskminsize, minimum width=\tskminsize} }
\tikzset{ stask/.style  = {draw=Paired-1,  rounded corners=0pt, text=Paired-1, minimum height=\tskminsize, minimum width=\tskminsize, fill=Paired-1!20} }
...
\tikzset{ sin/.style    = {draw=Paired-7,  circle, minimum width=\scksize, text=black, preaction={fill=white}, pattern=north east lines, pattern color=Paired-7} }
\tikzset{ sout/.style   = {draw=Paired-5,  circle, minimum width=\scksize, text=black, preaction={fill=white}, pattern=crosshatch dots, pattern color=Paired-5} }
\tikzset{ sfwd/.style   = {draw=Dark2-7,   circle, minimum width=\scksize, text=black, preaction={fill=white}, pattern=grid, pattern color=Dark2-7} }
...
% #1 node options (can be "noopt")
% #2 id {ti}
% #3 position {x,y} (can be "nopos")
...
\newcommand{\task}[7]{
  ...
  % draw task
  \def\tskpos{#3}
  \ifthenelse{\equal{\tskpos}{\string "nopos"}} {
    \ifthenelse{\equal{#1}{\string "noopt"}} {
      \node[task, align=center] (\tskid) {\tsklabl};
    }{
      \node[task, align=center, #1] (\tskid) {\tsklabl};
    }
  }{
    \pgfmathsetmacro{\tskposx}{{#3}[0]}
    \pgfmathsetmacro{\tskposy}{{#3}[1]}
    \ifthenelse{\equal{#1}{\string "noopt"}} {
      \node[task, align=center] (\tskid) at (\tskposx,\tskposy) {\tsklabl};
    }{
      \node[task, align=center, #1] (\tskid) at (\tskposx,\tskposy) {\tsklabl};
    }
  }
  ...
  \ifnum \numexpr\nsckif > 0
    ...
    \foreach \i in {0,...,\nsckifmone} {
      ...
      \ifnum \numexpr \i  < \nscki
        \node[sin, at=(\tskid.west), yshift=\sckiyshift] (\tskid_si\sckiid) {};
      \else
        \node[sfwd, at=(\tskid.west), yshift=\sckiyshift] (\tskid_si\sckiid) {};
      \fi
    }
  \fi
  ...
  \ifnum \numexpr\nsckof > 0
    ...
    \foreach \i in {0,...,\nsckofmone} {
      ...
      \ifnum \numexpr \i  < \nscko
        \node[sout, at=(\tskid.east), yshift=\sckoyshift] (\tskid_so\sckoid) {};
      \else
        \node[sfwd, at=(\tskid.east), yshift=\sckoyshift] (\tskid_so\sckoid) {};
      \fi
    }
  \fi
...
\newcommand{\legend}[1]{
  \begin{scope}[shift={#1}]
    ...
    \node[] (l8) at (11.15, 1.9) {};
    \node[draw, fit=(l1) (l2) (l3) (l4) (l5) (l6) (l7) (l8)] (legend) {};
  \end{scope}
}
