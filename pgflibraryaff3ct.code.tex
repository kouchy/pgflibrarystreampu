...
% #8 a list of attributes (can be {"rev", "seq", "noattr"})
\newcommand{\task}[8]{
  % parse arguments
  \def\tskid{#2}
  \def\tsklabl{#4}
  ...
  \foreach \i in {0,...,\nattrmone}{
    ...
    \ifthenelse{\equal{\curattr}{noattr}} {
      \breakforeach
    }{
  ...
  \ifthenelse{\equal{\tskpos}{\string "nopos"}} {
    \ifthenelse{\equal{#1}{\string "noopt"}} {
      \node[\tskstyle, align=center] (\tskid) {\tsklabl};
    }{
      \node[\tskstyle, align=center, #1] (\tskid) {\tsklabl};
    }
  }{
    \pgfmathsetmacro{\tskposx}{{#3}[0]}
    \pgfmathsetmacro{\tskposy}{{#3}[1]}
    \ifthenelse{\equal{#1}{\string "noopt"}} {
      \node[\tskstyle, align=center] (\tskid) at (\tskposx,\tskposy) {\tsklabl};
    }{
      \node[\tskstyle, align=center, #1] (\tskid) at (\tskposx,\tskposy) {\tsklabl};
    }
  }
