...
% #5 a list of attributes (can be {"lbup"} and can also be empty)
\newcommand{\module}[5][]{
  % parse arguments
  \pgfmathsetmacro{\ntsk}{dim(#2)}
  \def\tids{};
  ...
  \ifthenelse{\equal{#5}{}} {
    ...
    \foreach \i in {0,...,\nattrmone}{
      \ifthenelse{\nattrmone=0}{
        \pgfmathsetmacro{\curattr}{#5}
      }{
        \pgfmathsetmacro{\curattr}{{#5}[\i]}
      }
      \ifthenelse{\equal{\curattr}{lbup}} {
        \xdef\islabelup{yes}
      }{}
    }
  }
  ...
  % start drawing
  \ifthenelse{\equal{\islabelup}{yes}} {
    \node[module, label={[Paired-3,align=center]above:\mdllabl}, fit=\tids,#1] (\mdlid) {};
  }{
